...
export const registerServiceWorker = async (): Promise<ServiceWorkerRegistration> => {
  ...
  if (!import.meta.env.PROD) {
    throw new Error('El Service Worker solo se registra en producci贸n');
  }
...
export const getExistingSubscription = async (registration: ServiceWorkerRegistration) => {
  try {
    ...
    console.error('Error obteniendo suscripci贸n existente:', error);
    return null;
  }
};
...
export const saveSubscriptionToServer = async (subscription: PushSubscription) => {
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.error?.message || 'Error guardando suscripci贸n');
    }
...
export const removeSubscriptionFromServer = async () => {
  try {
    ...
    if (!response.ok) {
      throw new Error('Error removiendo suscripci贸n');
    }
...
export const setupPushNotifications = async (): Promise<PushSubscription> => {
  try {
    ...
    if (permission !== 'granted') {
      throw new Error('Permisos de notificaci贸n denegados. Para activar las notificaciones:\n\n1. Haz clic en el 铆cono de candado en la barra de direcciones\n2. Cambia "Notificaciones" a "Permitir"\n3. Recarga esta p谩gina');
    }
...
export const sendTestNotification = async () => {
  try {
    ...
    const options: NotificationOptionsCompat = {
      body: 'Esta es una notificaci髇 de prueba desde el navegador',

...
import { useUserAchievements } from '@/hooks/api/use-achievements';
...
const AchievementsGrid: React.FC<AchievementsGridProps> = ({ preview = false }) => {
  const { data: achievementsData, isLoading } = useUserAchievements();

  const achievements = achievementsData?.achievements || [];
  const stats = achievementsData?.stats || { total: 0, unlocked: 0, locked: 0 };
  ...
  const displayedAchievements = preview ? achievements.slice(0, 3) : achievements;
      ...
      {preview && achievements.length > 3 && (
        <div className="text-center">
          <p className="text-sm text-muted-foreground">
            Y {achievements.length - 3} logros más...
          </p>
        </div>
      )}
    </div>
